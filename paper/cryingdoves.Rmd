...
doves_korm <- read.csv(here::here("data/doves_korm_nomiss.csv"))
doves_korq <- read.csv(here::here("data/doves_korq.csv"))
...
```{r m123_plot, echo=FALSE, fig.height = 3, fig.cap = "\\label{fig:m123_plot}The Effect of External Relations on Quarterly Approval Ratings"}
m1_coef <- broom::tidy(Model1) %>%
  mutate(model = "Model 1")
m2_coef <- broom::tidy(Model2) %>% 
  mutate(model = "Model 2")
m3_coef <- broom::tidy(Model3) %>% 
  mutate(model = "Model 3")
m123_coef <- bind_rows(m1_coef, m2_coef, m3_coef) %>% 
  filter(term != "(Intercept)") %>% 
  mutate(order = case_when(term == "L(app_q)" ~ 1,
    term == "L(app_q, 4)" ~ 2,
    term == "kj_q_ts" ~ 3,
    term == "kc_q_ts" ~ 4,
    term == "kp_q_ts" ~ 5,
    term == "unemp_q_d_ts" ~ 6,
    term == "L(kp_q_ts)" ~ 7,
    term == "L(unemp_q_d_ts)" ~ 8,
    term == "newgov_q_ts" ~ 9)) %>% 
  arrange(order)

dwplot(m123_coef) %>% 
  relabel_predictors("L(app_q)" = "Approval(t-1)",
   "L(app_q, 4)" = "Approval(t-4)",
   "kj_q_ts" = "SK2JP",
   "kc_q_ts" = "SK2CH",
   "kp_q_ts" = "SK2NK",
   "unemp_q_d_ts" = "Unemployment",
   "L(kp_q_ts)" = "SK2NK(t-1)",
   "L(unemp_q_d_ts)" = "Unemployment(t-1)",
   "newgov_q_ts" = "New Government") +
  theme_bw() +
  scale_color_discrete(name="Approval\nRatings\n(Monthly)") +
